...
export default async function ClubPage({ params }: Props) {
  ...
  const deepLink = `rds://club/${encodeURIComponent(cleanCode)}`;
  ...
  return (
    ...
    >
      ...
      <h1 className="text-white font-black uppercase text-3xl md:text-4xl tracking-tight mb-3"
        ...
        Rejoins RDS avec ton club
      </h1>
      <p className="text-white/70 mb-10 max-w-sm text-base">
        Ton club t'invite sur Renard des Surfaces, le jeu de la Coupe du Monde.
      </p>
      ...
      <p className="text-white/40 text-xs">
        Code club&nbsp;: <span className="font-semibold tracking-[0.2em] text-white/70">{cleanCode}</span>
      </p>
      <p className="text-white/40 text-xs mt-1">
        Entre ce code à l'inscription dans «&nbsp;Code parrain ou club&nbsp;».
      </p>
    </main>
  );
}
